...
Layanan AI ini dilengkapi dengan fitur-fitur tingkat lanjut berikut:
...
Untuk menjalankan proyek ini di mesin lokal, siapkan konfigurasi *environment* pada masing-masing modul:
...
| :--- | :--- | :--- |
| `GEMINI_API_KEY` | API Key dari Google AI Studio untuk akses LLM | `masukkan_gemini_api_key` |
...
| Variabel | Deskripsi | Contoh Nilai |
| :--- | :--- | :--- |
| `GEMINI_API_KEY` | API Key untuk modul *Financial Summary* | `masukkan_gemini_api_key` |
...
Kedua layanan (Chatbot & Model API) dapat dijalankan menggunakan *virtual environment* Python di mesin lokal.
...
2. Buat dan aktifkan *virtual environment*, lalu instal dependensi:
   ...
   ```
3. **PENTING: Build Vector Database.** Sebelum menjalankan server, database WAJIB dibangun agar RAG dapat berfungsi:
   ```bash
   python build_db.py
   ```
   *(Script ini akan membaca `knowledge_base.json` dan menghasilkan folder `fundraise_vectordb/`)*
4. Setelah database berhasil di-build, jalankan server chatbot:
   ```bash
...
Proyek ini telah dikonfigurasi untuk berjalan di atas ekosistem **Hugging Face Spaces** menggunakan arsitektur **Docker**. Ikuti panduan berikut untuk melakukan deployment layanan secara mandiri:

### Langkah 1: Persiapan Repository
1. Buat *Space* baru di Hugging Face. Pilih lisensi yang sesuai dan atur visibilitasnya langsung ke **Public**.
2. Pada bagian *Space hardware*, cukup pilih **CPU basic**.
3. Pada opsi *Space SDK*, pilih **Docker** -> **Blank**.

### Langkah 2: Konfigurasi Secrets (Environment Variables)
Sebelum mengunggah kode, atur kredensial dan environment variables yang dibutuhkan:
1. Buka halaman *Space*, lalu masuk ke tab **Settings**.
2. Gulir ke bawah hingga menemukan bagian **Variables and secrets**.
3. Tambahkan *New secret* untuk setiap *environment variable* yang ada pada panduan "Petunjuk Setup Environment" sebelumnya (misal: `GEMINI_API_KEY`, `BACKEND_URL`, dan `API_KEY`).
   *Catatan: Pastikan menggunakan opsi rahasia (secret) agar nilai kunci API tidak terekspos.*

### Langkah 3: Mengunggah Modul Layanan
Hugging Face Spaces bekerja dengan mem-build image Docker berdasarkan struktur file yang ada pada direktori *root*. Buatlah *Space* terpisah untuk setiap layanan (1 Space untuk AI-Advisor, 1 Space untuk Model API, dan 1 Space untuk Monitoring Prometheus).

Unggah seluruh isi direktori dari modul yang ingin di-deploy ke dalam direktori *root* pada *Space* masing-masing.

**🚨 PENGECUALIAN PENTING (Khusus Modul AI Advisor Chatbot):**
Saat mengunggah isi folder `AI-Advisor-FundRaise/` ke *Space*, **JANGAN** mengunggah file `knowledge_base.json` dan `build_db.py`. 
Jika file tersebut ikut diunggah, sistem akan mencoba mem-*build* ulang Vector Database dan mengeksekusi *embedding* Gemini secara berulang di server Hugging Face, yang akan sangat cepat menghabiskan kuota token API. 

**Solusi:** Pastikan perintah `python build_db.py` sudah dijalankan di mesin lokal terlebih dahulu. Setelah itu, cukup unggah folder `fundraise_vectordb/` yang sudah jadi beserta `app.py`, `Dockerfile`, dan `requirements.txt` ke Hugging Face.

### Langkah 4: Proses Build dan Running
Setelah *file* terunggah atau ada perubahan terbaru (melalui *git push*), Hugging Face akan secara otomatis memulai proses *Building*. Jika terdapat kesalahan konfigurasi pada `Dockerfile`, rinciannya dapat dilihat pada menu "Logs". Jika berhasil, status akan berubah menjadi **Running**. 

Layanan API kini sudah siap diakses secara publik melalui URL yang diberikan oleh Hugging Face!
...
* **G-Drive Aset Model:** [Google Drive Folder - Model Files](https://drive.google.com/drive/folders/1ARBgCh-3UrBW-yZY1RJW0xTPc5kv0hnt?usp=sharing)

### Visualisasi Monitoring
Metrik pemantauan tingkat lanjut digunakan untuk mencegah masalah stabilitas sistem. Metrik yang dipantau meliputi waktu latensi per *request*, manajemen alokasi memori RAM *container*, dan laju arus data.
